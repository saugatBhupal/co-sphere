...
import 'get_job_by_id_usecase_test.mocks.dart';
...
void main() {
  ...
  test('should return the list of new jobs ', () async {
    ...
    when(mockJobRepository.getJobById(dto)).thenAnswer((_) async => Right(job));

    final result = await getJobByIdUsecase(dto);

    expect(result, Right(job));
    verify(mockJobRepository.getJobById(dto)).called(1);
  ...
  test('should return Failure when an error occurs', () async {
    ...
    when(mockJobRepository.getJobById(dto))
