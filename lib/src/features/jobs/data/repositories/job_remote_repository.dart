...
class JobRemoteRepository implements JobRepository {
  ...
  @override
  Future<Either<Failure, Job>> applyToJob(ApplyJobReqDto dto) async {
    try {
      final JobApiModel job = await datasource.applyToJob(dto);
      return Right(job.toDomain());
    } catch (e) {
      return Left(Failure(message: e.toString()));
    }
  ...
  @override
  Future<Either<Failure, List<Job>>> getExploreJobs(String uid) async {
    try {
      final List<JobApiModel> jobs = await datasource.getExploreJobs(uid);
      return Right(jobs.map((job) => job.toDomain()).toList());
    } catch (e) {
      return Left(Failure(message: e.toString()));
    }
